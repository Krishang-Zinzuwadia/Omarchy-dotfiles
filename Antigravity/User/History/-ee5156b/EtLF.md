...
**Possible Causes:**
1.  **Schema Mismatch:** The `UIElement` class definition (in `isr_schema.py` or similar) does not match what the VLM backend or Planner expects.
2.  **Version Incompatibility:** The VLM backend might be returning data that the current schema doesn't support, or vice versa.

**Troubleshooting:**
-   Inspect the `UIElement` class definition.
-   Check the VLM backend's output format.
-   This is usually a code bug, not a runtime environment issue.
