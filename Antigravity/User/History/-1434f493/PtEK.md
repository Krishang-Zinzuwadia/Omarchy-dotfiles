...
- **Note:** Do not use `pyautogui` for screenshots in production agents; it is slower and less robust. Use `pyautogui` only for mouse/keyboard control.
...
def capture_windows(output_path):
    try:
        ...
        raise ImportError("mss is required for Windows screenshots. Install with: pip install mss")
```
