...
- Add `pyautogui>=0.9.50` (for mouse control only, NOT for screenshots)
...
- **Update** `_capture_linux`:
    -   **Exclusive** use of `grim` for Wayland. Remove other fallbacks if `grim` fails (fail fast).
-   **Update** `_capture_windows`:
    -   **Exclusive** use of `mss`. Remove `pyautogui` screenshot fallback.
-   **Update** `_capture_macos`:
    -   Use `screencapture` (native) or `mss` if available. Remove `pyautogui` screenshot fallback.
